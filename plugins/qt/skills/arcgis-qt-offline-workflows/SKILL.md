---
name: arcgis-qt-offline-workflows
description: Implement or improve offline map workflows with ArcGIS Maps SDK for Qt. Use for local data, mobile map packages, offline map areas, file storage, and disconnected user experiences in Qt apps.
license: MIT
---

Use this skill when the request involves **offline GIS behavior** in a Qt app.
...
3. Fit ArcGIS jobs and async work into the app's existing threading, signals, or model-update style.
4. Make status, cancellation, retry, and stale-data behavior explicit.
5. Keep local storage paths and cleanup rules predictable across target platforms.
...
- Prefer a small coordinator, controller, or model for offline orchestration instead of burying everything in UI code.
